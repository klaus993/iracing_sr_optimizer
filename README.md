...
Fetch actual race results for the current week to see real-world incident rates and SR changes:
...
| Column | Description |
|--------|-------------|
| AvgInc | Average incident points per driver |
| AvgSR | Average SR change (e.g., +0.12 or -0.05) |

### Personal SR prediction

Predict how each series will affect your SR based on your current CPI:

```bash
# Manual SR input
python -m iracing_sr_optimizer --week 8 --my-sr C3.45

# Auto-fetch from iRacing API using customer ID
python -m iracing_sr_optimizer --week 8 --cust-id 123456

# Or set via environment variable
export IRACING_CUST_ID="123456"
python -m iracing_sr_optimizer --week 8
```

When personal SR is provided along with empirical data, a `Pred` column appears showing `+` (SR likely up), `-` (SR likely down), or `~` (neutral). The prediction compares your CPI against the series' empirical average CPI.
...
`--json` and `--csv` output the same data with additional fields like `license_class`, `races_per_hour`, `incident_dq`, `is_heat_racing`, and the empirical fields (`avg_incidents`, `avg_sr_delta`, `empirical_sample_size`, `predicted_sr_direction`).
...
iracing_sr_optimizer/
  ...
  fetch_results.py     # Fetches past race results, aggregates empirical SR data
  models.py            # Series, WeekSchedule, SRPotential, SeriesEmpirical dataclasses
  sr_calculator.py     # SR scoring engine
  sr_predictor.py      # Personal SR prediction and --my-sr parsing
...
data/
  ...
  results_cache/       # Cached empirical results per season/week (generated by --fetch-results)
tests/
  ...
  test_fetch_results.py    # Empirical data aggregation
  test_sr_predictor.py     # SR parsing and prediction logic
  test_integration.py      # Full pipeline and CLI tests
```
